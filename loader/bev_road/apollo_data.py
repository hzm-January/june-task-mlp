...
class Apollo_dataset_with_offset(Dataset):
    ...
    def get_y_offset_and_z(self, res_d):
        ...
        for idx in res_d:
            ...
            ipm_points = ipm_points_.T[
                np.where((ipm_points_[1] >= 0) & (ipm_points_[1] < self.ipm_h))].T  # 进行筛选 x [0,(103-3)/0.5=200]
    ...
    def get_seg_offset(self, idx):
        ...
        for lane_idx in range(len(lane_grounds)):
            ...
            lane_ground = np.concatenate([lane_ground, np.ones([lane_ground.shape[0], 1])],
                                         axis=1).T  # lane_ground(4,71)
            # get image gt
            ...
            # matrix_IPM2ego[:, :2] [[-0.5  0. ], [ 0.  -0.5]] 求逆 [[-2. -0.], [-0. -2.]]
