...
class PatchEmbedOverlapping(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x)  # x (8,3,567,1024) -> (8,64,144,256)
        x = self.norm(x)  # x (8,64,144,256) -> (8,64,144,256)
        return x
...
class WaveNet(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            print('mmseg or mmdet init_weight')
    ...
    def forward(self, x):
        x = self.forward_embeddings(x)  # (8,3,576,1024) -> (8,64,144,256)
        x = self.forward_tokens(x)  # (8,64,144,256) -> (8,512,18,32)
        if self.fork_feat:
            return x
        x = self.norm(x)  # (8,512,18,32) -> (8,512,18,32)
        cls_out = self.head(F.adaptive_avg_pool2d(x, output_size=1).flatten(1))  # x.mean(1) # (8,512,18,32)->(8,512,1,1)->(8,512)
        return cls_out  # (8,1000)
